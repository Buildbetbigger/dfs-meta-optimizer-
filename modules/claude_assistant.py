...
You understand player psychology, recency bias, narrative angles, and market behavior."""
        ...
        prompt = f"""Predict the projected ownership for this DFS player:
...
1. **Recency Bias**: Did they have a big game recently? (Drives ownership up)
2. **Salary Value**: Are they underpriced for their projection? (High ownership)
3. **Narrative Angles**: Revenge game, primetime, bounce-back spot? (Increases ownership)
4. **Visibility**: National TV game or afternoon slate? (TV = higher ownership)
5. **Injury News**: Coming back from injury or dealing with one? (Complex)
6. **Team Context**: Popular team or small market? (Popular teams = higher ownership)
7. **Position Scarcity**: Is this position thin? (Can drive ownership)

Predict ownership percentage (0-100) and explain your reasoning.

Respond in ONLY valid JSON format:
{{
    "ownership": <number between 0-100>,
    ...
    "risk": "<what could change this prediction>"
        ...
        try:
            response = self._call_claude(prompt, system)def predict_ownership(self, 
                     player_data: Dict,
                     context: Dict = None) -> Dict:
    """
    Use Claude to predict player ownership percentage
...
You understand leverage, field dynamics, and optimal tournament strategy."""
        ...
        top_leverage = player_metrics.nlargest(8, 'leverage')[
            ['name', 'ownership', 'projection', 'leverage']].to_string()
